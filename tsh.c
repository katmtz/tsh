...
eval(char *cmdline)
{
    ...
    if (!check_builtins(&tok)) {
        ...
        if (pid < 0) {
            if (errno == EAGAIN) {
                unix_error("Error: Too many processes to fork process.\n");
            } else if (errno == ENOMEM) {
                unix_error("Error: Not enough memory to fork process.\n");
            } else {
...
/*
 * check_builtins - case/switch that checks for any builtin functions being
 *     called and evaluates them. (called from eval only!)
 * returns 1 if it has evaluated something
 */
